...
submit.onclick = function () {
  ...
  if (newPro.title.length == 0) {
    window.alert("you must enter title");
    title.focus();
  } else if (newPro.price.length == 0) {
    window.alert("you must enter price");
    price.focus();
  } else if (newPro.category.length == 0) {
    window.alert("you must enter category");
    category.focus();
  } else if (newPro.count < 1) {
    window.alert("count must be 1 or more");
    count.focus();
  } else {
    if (mood === "create") {
      // count
  ...
  for (let i = 0; i < dataPro.length; i++) {
    table += `
            <tr>
                ...
                <td><button onclick = "sellItem(${i})" id="sell">Sell</button></td>
            </tr>
        `;
  }
